...
CORR_DIR = "citation_correlations"
REF_FILE = "corr_citation.csv"

PLOTS_DIR = "citation_plots"
...
plt.title("Eigenvalue spectrum (citation network)")
plt.legend(fontsize=8)
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "eigenvalue_spectrum.png"))
plt.close()
...
plt.title("Cumulative variance (citation network)")
plt.legend(fontsize=8)
plt.tight_layout()
...
plt.ylabel("Cosine similarity with citation v1")
plt.title("Leading eigenvector alignment")
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "eigenvector_alignment.png"))
plt.close()
...
plt.title("Dominant observables in leading mode (citation)")
plt.tight_layout()
plt.savefig(os.path.join(PLOTS_DIR, "leading_mode_observables.png"))
plt.close()

print("All citation plots saved to", PLOTS_DIR)
